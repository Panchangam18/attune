...
function cmdPatch(query) {
    ...
    try {
        ...
        console.error(`Failed to patch "${app.name}":`, e.message);
        process.exit(1);
    }
}
function cmdUnpatch(query) {
    if (!query) {
    ...
    try {
        ...
        console.error(`Failed to unpatch "${app.name}":`, e.message);
        process.exit(1);
    }
}
function cmdSetCSS(query, cssFilePath) {
